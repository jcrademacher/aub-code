function raw_data = LTspice2Matlab2( filename, varargin )
%
%
...
%           Bugfix for newer software versions:     Daan de Rooij 09/07/2022 (Verified with Matlab R2017a and LTspice XVII)
%           Bugfix change list:    
%               - the_line = fgetl(fid); -> the_line =(strrep(fgetl(fid),native2unicode(0),''));
%               - fid = fopen(filename, 'rb'); -> fid = fopen(filename, 'r','b');
%               - fid = fopen(sprintf( '%s.raw', filename ), 'rb'); -> fid = fopen(sprintf( '%s.raw', filename ), 'r','b');
% 
%    Calling Convention:
